...
public:
  ...
  virtual void move(vector<vector<int> > &map, int maxx, int maxy)
  {
    ...
    if(dist < 20) {
      int caseNum = rand() % 9 + 1;
      if(caseNum == 1)    p = p + Point(0, -1);       // ↑
      else if(caseNum == 2)   p = p + Point(1, -1);   // ↗
      else if(caseNum == 3)   p = p + Point(1, 0);    // →
      else if(caseNum == 4)   p = p + Point(1, 1);    // ↘
      else if(caseNum == 5)   p = p + Point(0, 1);    // ↓
      else if(caseNum == 6)   p = p + Point(-1, 1);   // ↙
      else if(caseNum == 7)   p = p + Point(-1, 0);   // ←
      else if(caseNum == 8)   p = p + Point(-1, -1);  // ↖
      else p = p;
